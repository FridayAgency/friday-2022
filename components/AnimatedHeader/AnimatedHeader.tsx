...
export const AnimatedHeader: React.FC = () => {
	...
	useEffect(() => {
		...
		gsap.set(marketingRef.current, { y: marketingCor.start.y, x: marketingCor.start.x });
		gsap.set(plusRef.current, { y: plusCor.start.y, x: plusCor.start.x });
		gsap.set(experienceRef.current, { y: experienceCor.start.y, x: experienceCor.start.x });
		gsap.set(equalsRef.current, { y: equalsCor.start.y, x: equalsCor.start.x });
		gsap.set(outcomesRef.current, { y: outcomesCor.start.y, x: outcomesCor.start.x });
		...
		tl.to(marketingRef.current, {
			...
			.to(
				equalsRef.current,
				{ opacity: 1, y: equalsCor.middle.y, duration: 0.25, ease: Power1.easeIn },
				"=+0.4"
			)
			...
			.to([fadeTwo.current, fadeThree.current], { opacity: 1, duration: 0.25, ease: Power1.easeIn })
			.to([fadeFour.current, fadeFive.current], { opacity: 1, duration: 0.25, ease: Power1.easeIn })
			.to([fadeSix.current, fadeSeven.current], { opacity: 1, duration: 0.25, ease: Power1.easeIn })
			.to(fadeEight.current, { opacity: 1, duration: 0.25, ease: Power1.easeIn });
	}, []);
